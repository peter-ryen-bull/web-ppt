...
const SPORSMAL: { x: number; y: number; text: string; lineTo: [number, number]; at: number }[] = [
  { x: 300, y: 116, text: "Hva betyr feltene?", lineTo: [290, 150], at: 1 },
  { x: 92, y: 462, text: "Hvor ferske er dataene?", lineTo: [140, 402], at: 1 },
  { x: 268, y: 512, text: "Hvem svarer når noe ser rart ut?", lineTo: [330, 402], at: 1 },
];
...
const EGENSKAPER: { title: string; sub: string; icon: ReactNode }[] = [
  ...
  { title: "Tydelig eier", sub: "team-ocean svarer", icon: <IkonPerson /> },
  ...
  return (
    ...
    >
      ...
      {EGENSKAPER_VENSTRE.map((e, i) => (
        <Egenskap key={e.title} x={650} y={380 + i * 70} w={250} at={2} {...e} />
      ))}
      {EGENSKAPER.map((e, i) => (
        <Egenskap key={e.title} x={920} y={195 + i * 77} w={250} at={2} {...e} />
      ))}
